<template>
  <div
    ref="container"
    :class="['mt-image', $attrs.class]"
    :style="$attrs.style"
  >
    <slot v-if="loading" name="placeholder">
      <div class="mt-image__placeholder"></div>
    </slot>
    <slot v-else-if="hasLoadError" name="error">
      <div class="mt-image__error">{{ t('el.image.error') }}</div>
    </slot>
    <img
      v-else
      class="mt-image__inner"
      v-bind="attrs"
      :src="src"
      :style="imageStyle"
      :class="{ 'mt-image__inner--center': alignCenter, 'mt-image__preview': preview }"
      @click="clickHandler"
    >
...
import { useAttrs } from '@mtui/hooks'
import isServer from '@mtui/utils/isServer'
import { on, off, getScrollContainer, isInContainer } from '@mtui/utils/dom'
import { t } from '@mtui/locale'
import ImageViewer from '@mtui/image-viewer'
