...
<template>
  <mt-space wrap>
    <mt-card class="box-card" style="width: 250px" v-for="i in 3" :key="i">
      <template #header>
        <div class="card-header">
          <span>Card name</span>
          <mt-button class="button" type="text">Operation button</mt-button>
        </div>
      </template>
...
```html
<template>
  <mt-space direction="vertical">
    <mt-card class="box-card" style="width: 250px" v-for="i in 2" :key="i">
      <template #header>
        <div class="card-header">
          <span>Card name</span>
          <mt-button class="button" type="text">Operation button</mt-button>
...
```html
<template>
  <mt-space direction="vertical" alignment="start" :size="30">
  <mt-radio-group v-model="size">
    <mt-radio :label="'mini'">迷你</mt-radio>
    <mt-radio :label="'small'">小</mt-radio>
    <mt-radio :label="'medium'">中等</mt-radio>
    <mt-radio :label="'large'">大号</mt-radio>
  </mt-radio-group>

  <mt-space wrap :size="size">
    <mt-card class="box-card" style="width: 250px" v-for="i in 3" :key="i">
      <template #header>
        <div class="card-header">
          <span>Card name</span>
...
```html
<template>
 <mt-space :size="size" :spacer="spacer">
    <div v-for="i in 2" :key="i">
      <mt-button>
        button {{ i }}
      </mt-button>
...
import { MtDivider } from 'mtui'
export default {
  data() {
    return {
      size: 10,
      spacer: h(MtDivider, { direction: 'vertical' }),
    }
  },
}
</script>
...
| prefixCls | 给 space-items 的类名前缀 | string | mt-space | - |
